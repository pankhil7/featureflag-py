...
from app.models import FlagResponse, CreateFlagRequest, UpdateFlagRequest


def _row_to_flag(row: dict) -> FlagResponse:
    return FlagResponse(
        id=str(row["id"]),
        key=row["key"],
        enabled=row["enabled"],
...
def create(req: CreateFlagRequest) -> FlagResponse:
    conn = get_connection()
    try:
        with conn.cursor() as cur:
            ...
            return _row_to_flag(cur.fetchone())
    finally:
        conn.commit()
        conn.close()


def list_flags(env: Optional[str] = None) -> list[FlagResponse]:
    conn = get_connection()
    try:
        with conn.cursor() as cur:
            if env:
...
def get_by_key(key: str, env: Optional[str] = None) -> Optional[FlagResponse]:
    conn = get_connection()
    try:
        with conn.cursor() as cur:
            if env:
...
def update(key: str, env: str, req: UpdateFlagRequest) -> Optional[FlagResponse]:
